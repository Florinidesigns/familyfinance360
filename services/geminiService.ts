...
export const getFinancialAdvice = async (state: FinanceState): Promise<string> => {
  ...
  const prompt = `
    ...
    Rendimento Mensal: ${totalIncome}€
    Gastos Correntes (Presente): ${totalExpenses}€
    Compromissos de Longo Prazo (Passado - Mensal): ${totalMonthlyDebt}€
    Total em Poupanças (Futuro): ${state.goals.reduce((acc, g) => acc + g.currentAmount, 0)}€
    
    Dívidas Atuais:
    ${state.debts.map(d => `- ${d.name}: Falta pagar ${d.remainingValue}€ (${d.monthlyPayment}€/mês)`).join('\n')}
