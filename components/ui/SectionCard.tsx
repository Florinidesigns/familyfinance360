...
const SectionCard: React.FC<Props> = ({
    ...
    const variantStyles = {
        white: 'bg-white border-slate-100 text-slate-800',
            ...
            {(title || icon) && (
                <div className="flex justify-between items-start mb-8 relative z-10">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            {title && <h3 className={`text-xl md:text-2xl font-black ${variant === 'white' ? 'text-slate-800' : 'text-white'}`}>{title}</h3>}
                            {subtitle && <p className={`text-sm opacity-60 max-w-md ${variant === 'white' ? 'text-slate-500' : 'text-white/80'}`}>{subtitle}</p>}
                        </div>
                    </div>
