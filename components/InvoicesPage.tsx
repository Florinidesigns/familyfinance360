    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 transition-colors duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[40px] shadow-sm border border-slate-100 dark:border-slate-800">
          ...
          <p className="text-4xl font-black text-slate-800 dark:text-white">{coveragePercent}%</p>
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[40px] shadow-sm border border-slate-100 dark:border-slate-800">
          ...
          <p className="text-4xl font-black text-slate-800 dark:text-white">{totalInvoiced.toLocaleString('pt-PT')}{currencySymbol}</p>
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-2 font-medium">{invoices.length} documentos registados</p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[40px] shadow-sm border border-slate-100 dark:border-slate-800">
          ...
          <p className="text-4xl font-black text-slate-800 dark:text-white">{pendingInvoices}</p>
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-2 font-medium">Gastos sem número de fatura</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[40px] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
        <div className="p-8 border-b border-slate-50 dark:border-slate-800 flex justify-between items-center bg-slate-50/30 dark:bg-slate-800/50">
          <h3 className="text-xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
            <FileText size={20} className="text-emerald-600 dark:text-emerald-400" /> Arquivo Digital de Faturas
          </h3>
          <span className="text-[10px] bg-slate-900 dark:bg-emerald-600 text-white px-4 py-1.5 rounded-full font-black uppercase tracking-widest">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-50/50 dark:bg-slate-800/30">
              <tr>
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">Fatura Nº</th>
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">Estabelecimento</th>
                ...
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] text-right">Valor</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
              {invoices.map(invoice => (
                <tr key={invoice.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all group">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-black text-slate-800 dark:text-slate-200 tracking-tight">{invoice.invoiceNumber}</span>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-2 text-slate-600 dark:text-slate-400 font-bold text-sm">
                      <Store size={14} className="text-slate-300 dark:text-slate-600" /> {invoice.establishment || 'Geral'}
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-2 text-slate-500 dark:text-slate-600 text-xs font-medium">
                      <Calendar size={14} /> {invoice.date}
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <span className="flex items-center gap-2 text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-full w-fit font-bold text-[10px] uppercase tracking-tighter">
