    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white w-full max-w-2xl rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
        <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
          <div><h3 className="text-2xl font-black text-slate-800">{t.editModal.title}</h3><p className="text-slate-400 text-sm font-medium">{t.editModal.subtitle}</p></div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex bg-slate-50 p-1.5 rounded-2xl border border-slate-100">
              <button type="button" onClick={() => { setType('entrada'); setCategory(INCOME_SOURCES[0]); handleInvoiceToggle(false); }} className={`px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${type === 'entrada' ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}>{t.editModal.entry}</button>
              <button type="button" onClick={() => { setType('saida'); setCategory(CATEGORIES[0]); }} className={`px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${type === 'saida' ? 'bg-rose-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}>{t.editModal.exit}</button>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase text-slate-400 ml-1">{t.editModal.category}</label>
              <select className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-4 font-bold text-slate-700 outline-none" value={category} onChange={(e) => setCategory(e.target.value as any)}>
                {type === 'entrada' ? INCOME_SOURCES.map(s => <option key={s} value={s}>{t.incomeSources[s as keyof typeof t.incomeSources] || s}</option>) : CATEGORIES.map(c => <option key={c} value={c}>{t.categories[c as keyof typeof t.categories] || c}</option>)}
              </select>
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase text-slate-400 ml-1">{t.editModal.description}</label>
          ...
          {type === 'saida' && category === 'Alimentação' && (
            <div className="space-y-2"><label className="text-[10px] font-black uppercase text-slate-400 ml-1">{t.editModal.establishment}</label>
            ...
            <div className={`p-6 rounded-3xl border transition-all space-y-4 ${hasInvoice ? 'bg-emerald-50 border-emerald-200' : 'bg-slate-50 border-slate-100'}`}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs font-black uppercase tracking-widest text-slate-500">{t.editModal.fiscalInvoice}</span>
                </div>
                {hasInvoice && (
                  <div className="flex items-center gap-2 px-3 py-1.5 bg-white rounded-xl border border-emerald-100">
                    ...
                    <label htmlFor="edit-no-nif" className="text-[10px] font-black uppercase text-slate-500 flex items-center gap-1 cursor-pointer"><UserMinus size={12} /> {t.editModal.noNif}</label>
                  </div>
                )}
              </div>
              {hasInvoice && !isNoNif && (
                <div className="relative"><FileText className="absolute left-4 top-1/2 -translate-y-1/2 text-emerald-600" size={16} /><input type="text" className="w-full bg-white border border-emerald-100 rounded-xl pl-10 pr-4 py-2.5 text-xs font-bold text-emerald-700 outline-none" placeholder={t.editModal.invoiceNumber} value={invoiceNumber} onChange={(e) => setInvoiceNumber(e.target.value)} /></div>
              )}
            </div>
          )}

          <div className="pt-4 flex flex-col md:flex-row gap-4">
            <button type="button" onClick={onClose} className="px-6 bg-slate-100 text-slate-500 py-5 rounded-3xl font-black text-xs uppercase tracking-[0.2em] order-3 md:order-1">{t.common.cancel}</button>
            {onDelete && <button type="button" onClick={() => onDelete(transaction.id)} className="px-6 bg-rose-50 text-rose-600 py-5 rounded-3xl font-black text-xs uppercase tracking-[0.2em] flex items-center justify-center gap-2 order-2"><Trash2 size={18} /> {t.common.delete}</button>}
            <button type="submit" className="flex-1 bg-slate-900 text-white py-5 rounded-3xl font-black text-xs uppercase tracking-[0.2em] shadow-xl hover:bg-emerald-600 flex items-center justify-center gap-2 order-1 md:order-3 active:scale-95"><Check size={20} className="text-emerald-400" /> {t.editModal.saveChanges}</button>
          </div>
        </form>
      </div>
    </div>
